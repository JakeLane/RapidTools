 ...
 * 
 * FireworkEffectPlayer provides a thread-safe and (reasonably) version independent way to instantly
 * explode a FireworkEffect at a given location. You are welcome to use, redistribute, modify and
 * destroy your own copies of this source with the following conditions:
 * 
...
public class FireworkEffectPlayer {
	 ...
	 * fplayer.playFirework(event.getPlayer().getWorld(), event.getPlayer.getLocation(),
	 * Util.getRandomFireworkEffect()); }
	 * 
	 * }
	 */
	...
	public void playFirework(World world, Location loc, FireworkEffect fe) throws Exception {
		...
		// null checks are fast, so having this separate is okay.
		if (nms_world_broadcastEntityEffect == null) {
			// get the method of the nms_world
			nms_world_broadcastEntityEffect = getMethod(nms_world.getClass(), "broadcastEntityEffect");
		}
		/*
		 * Now we mess with the metadata, allowing nice clean spawning of a pretty firework (look,
		 * pretty lights!)
		 */
		// metadata load
